...
## 2026-08-15：四頁型獨立實測與三項修復

四個 agent 各拿一個頁型、在各自獨立的專案裡從 `init` 走到 `visual_check`，
並逐張看過渲染 PNG。四份結論高度一致：**流程都能跑通、成品都能用，但把關機制是假的**
——四個人都各自做出了跑版的頁面，而 `svg_quality_checker` 全部回報 0 errors 並成功匯出。

### 契約宣稱 vs 實測

| 頁型 | 契約原本寫 | 實測真值 | 落差 |
|---|---|---|---|
| 對比頁 | 3–5 面向、18 字 | 屬實；18 字只佔可用寬 85% | 保守但方向對 |
| SWOT | 每象限 2–4 條、28 字 | **5 條、29 字**（第 31 字越過卡片框線） | 留了一條餘量 |
| KPI 儀表板 | 數值 6 個半形字元 | **11 個**（384 欄）、**14 個**（588 欄） | 低估近一半 |
| 流程圖 | 7 節點、步驟名 10 字 | 7 節點放不下四個欄位；步驟名 **8 字** | 高估 |

四份契約都寫過同一句「上表的字數上限已含 15% 餘裕」——**四個實測沒有一個是 15%**
（SWOT 0.7%、KPI 45%）。那句話是抄過去的，不是量出來的，已全數刪除。

### 修復 1：多段落文字框的寬度落差（`06-paragraph-latin-headroom.patch`）

**根因**（在引擎原始碼裡對到數字，非推測）：`elements.py` 對單行框加 6% headroom
（`_TEXT_WIDTH_HEADROOM_BASE`），對多段落框刻意不加——因為多段落框把該值當**換行約束**，
加了會把合併後的框撐得比作者的原始行寬大。這個取捨本身正確，但留下缺口：多段落框的寬度
取自「區塊內最寬的那一行」，而 CJK 估算精準、含數字或拉丁的行低估約 3%。
**只要最寬那行剛好中西混排，就是 0% 餘裕碰上 3% 低估，必定被折行。**

處置：含非 CJK 字元的行整行 ×1.04，純中文一個 px 都不動。**沒有把 `include_headroom`
翻成 `True`**——那會連 CJK 一起撐寬 6%，正是上游擔心的事。

補償按**整行**算而非按字元算：第一版只對非 CJK 字元加 4%（僅 1.2px）實測無效，
才確認落差來自 LibreOffice 在**中西交界插入的間距**，不是拉丁字元本身的寬度。

回歸：KPI 跑版案例修復（「會員組負／責」併回單行）；流程圖 3 頁 pixel-identical；
12 頁 deck 有 3 頁變化且全部是原本斷行處被修好，無一頁變壞。

### 修復 2：`spec_lock.md` 的 scaffold 落差（`overlay/scaffold/spec_lock.py`）

引擎的 `scaffold-lock` 只給 `body`／`title` 兩個字級與四個顏色鍵。**四個 agent 全部被同一個
錯誤擋下**：
...
**沒有做成第 11 個 patched file。** 判斷是上游沒錯——它給的是引擎通用的最小集合，不知道
shiftdeck 加了頁型庫；是頁型庫多要了欄位卻沒提供對應 scaffold，該由覆蓋層補。新工具先讓
引擎產生基底（schema 跟著上游走），再掃 `overlay/pagetypes/*/skeleton.svg` 得出五個字級
（54/40/24/18/14）與八個色角色，並依 `svg_output/` 列出 `page_rhythm`。新增頁型時自動跟上。

**順帶發現**：`## colors` 的值後面不能加行內註解。`load_theme_color_spec` 會把 `:` 後面整段
丟給 `parse_hex_color`，一個註解就讓那行被靜默跳過；全部跳過的話閘門會報
`theme contract is missing: colors`。

### 修復 3：四份契約用實測值改寫

- 共用的「跨軟體字型度量」整段重寫。原文寫「混排一律低估 7-10%、LibreOffice 會強制重斷行」，
  **兩個宣稱都不對**：小字級（14–24px）中西混排實際是**高估** 7–9%（正黑體拉丁實測約
  0.48–0.62 em），只有大字級粗體（40px 以上）低估約 5%
- **單行與多行的失敗方式不同**，這比字數上限重要：單行 `<text>` 匯出成 `wrap="none"`，
  LibreOffice 忽略該屬性，**文字直接溢出、不會斷行**；多行 tspan 合併成多段落框才**會真的斷行**
- 多行文字的實務規則：**讓最寬的那一行是純中文**。最寬行含數字或英文時，補一行更長的純中文
  把框撐開——要修的是別行，不是出問題的那一行
- 字數不是可靠單位：35 字中西混排 ≈ 21 字純中文的寬度
- KPI 數值上限 6 → 11／9 → 14 個半形字元；SWOT 條目 28 → 29 字；流程圖步驟名 10 → 8 字
- 補齊流程圖**排法 B 的箭頭座標**（原本三條規則在雙列欄距 40px 下無解：`12+22=34 > 40−8=32`）

### 尚未解決（誠實邊界）

1. **閘門只驗作者自己宣告的 bounds，不驗版面**。實測把兩張卡的 bounds 疊 28px、SWOT 每象限
   塞 7 條擠爆版面、文字越過卡片框線但仍在 bounds 內——一律 0 errors 並成功匯出。
   目前唯一能抓到跑版的仍然是人眼看 `visual_check.py` 的 PNG。
2. **對比頁的延遲引信**：`annotation` 字級只出現 2 次時被判 sparse 而放行，做到第 3 頁才會
   突然變 error。兩頁測過不代表五頁會過。
3. **KPI 的 checker 誤報偏保守**：對純數字字串高估約 17%（11 字元估 368.3px、實際 314.8px），
   會在還很安全時就給 `overflow horizontal` 的 WARN。
4. **流程圖 bounds 盲區**：有出向箭頭的節點 bounds 310 而卡片只有 270，中間 40px 完全沒有守門人。
5. **非工程師順暢度平均 2.25/5**（四份各給 2/2/3/2）。四份的扣分理由一致：檢查通過不等於
   版面正確，而最後把關全靠人眼逐張看圖。

### 同批的其他改動

- `05-preview-eager-load.patch`：18 張風格預覽縮圖原本是 `loading="lazy"`，使用者切到視覺方向
  區塊或改選項時圖才開始下載解碼，體感是「預覽慢半拍」。改為即時載入＋整份目錄預抓。
  實測（未捲動）：頁面載入即請求 18 張，DOM 內 21 個 img 全部 `eager` 且 `complete`。
- `overlay/console/`：瀏覽器主控台。開新簡報、看專案、看 agent 在忙什麼；瀏覽器按鈕寫請求進
  收件匣，agent 用 `agent.py wait` 等到它。**邊界**：`wait` 會佔住 agent 一輪且有 timeout，
  時間到之後請求只會排著沒人處理——主控台會明說「等待 agent 接手」而不是假裝在跑。
